...
int	pec_check_bonus(t_data *var, int i, int j)
{
	while (var->map[++i])
	{
		j = -1;
	...
	if (!var->collect || !var->exit || var->player != 1 || !var->enemy)
		return (0);
	return (1);
}
...
int	check_map_bonus(t_data *var)
{
	...
	var->derection = 'D';
	var->der = 1;
	if (!rectangle(var->map) || !wall(var->map) || \
		!pec_check_bonus(var, -1, -1))
	{
		ft_printf("Error\n");
